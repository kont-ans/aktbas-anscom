...
async def cb_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    record_id = db.set_active_number(user_id, phone_display, slug, country_name,
                                     expires_at, seen_csv=seen_csv, kind="free")

    _start_polling_job(context, user_id, chat_id, record_id, slug, digits,
                       phone_display, country_name, SESSION_DURATION_SEC)
...
def _start_polling_job(context, user_id, chat_id, record_id, slug, digits,
                       phone_display, country_name, duration):
    if not context.job_queue:
        return
    for job in context.job_queue.get_jobs_by_name(f"poll:{user_id}"):
...
async def poll_messages_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    slug = data["slug"]
    digits = data["phone_digits"]
    phone_display = data["phone_display"]
    duration = data.get("duration", SESSION_DURATION_SEC)
    ...
    for m in reversed(new_msgs[:5]):
        try:
            ...
            logger.warning("Send new msg failed: %s", e)
...
async def cmd_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _ensure_user(update): return
    user_id = update.effective_user.id
    _stop_user_session(context, user_id)
    user = db.get_user(user_id) or {}
...
async def cb_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    _stop_user_session(context, q.from_user.id)
    await show_main_menu(q, context, q.from_user.id)


def _stop_user_session(context, user_id: int):
    db.stop_active_number(user_id)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "paid_approve_input":
        ...
        # Start polling job for that user (their chat is their user_id for private chat)
        target_user = order["user_id"]
        chat_id = target_user
        _start_polling_job(context, target_user, chat_id, record_id, slug, digits,
                           phone_display, country_name, PAID_SESSION_DURATION_SEC)
...
async def post_init(app: Application):
    ...
    # Send all source files to each admin on startup
    import os
    _HERE = os.path.dirname(os.path.abspath(__file__))
    _FILES_TO_SEND = [
        "bot.py", "config.py", "database.py", "scraper.py",
        "requirements.txt", "admin_panel.py", "index.html",
    ]
    # رابط لوحة الإدارة الخارجية
    _replit_domain = os.environ.get("REPLIT_DEV_DOMAIN", "")
    if _replit_domain:
        _panel_url = f"https://{_replit_domain}/"
    else:
        _panel_url = f"http://localhost:{config.ADMIN_PANEL_PORT}/"

    for admin_id in ADMIN_IDS:
        try:
            await app.bot.send_message(
                admin_id,
                f"🚀 *البوت v3 يعمل الآن* — @{me.username}\n\n"
                f"🌐 *رابط لوحة الإدارة الخارجية:*\n"
                f"{_panel_url}\n\n"
                f"✨ *ميزات v3:*\n"
                f"  🔧 وضع الصيانة\n"
                f"  ⬇️ تعطيل الإيداع\n"
                f"  💎 تعطيل الأرقام المدفوعة\n"
                f"  📱 كود دخول عبر تيليجرام\n\n"
                f"📁 إرسال ملفات المشروع...",
                parse_mode="Markdown",
            )
            for fname in _FILES_TO_SEND:
            ...
            logger.warning("Could not send files to admin %s: %s", admin_id, e)


def main():
    db.init_db()

    # v3: تشغيل لوحة الإدارة الخارجية في خلفية
    try:
        from admin_panel import run_admin_panel
        run_admin_panel()
        logger.info("🌐 لوحة الإدارة الخارجية تعمل على المنفذ %s", config.ADMIN_PANEL_PORT)
    except Exception as e:
        logger.warning("فشل تشغيل لوحة الإدارة: %s", e)
    ...
    logger.info("✅ بوت الأرقام المؤقتة (نسخة v3 — مع لوحة الإدارة الخارجية) يعمل...")
